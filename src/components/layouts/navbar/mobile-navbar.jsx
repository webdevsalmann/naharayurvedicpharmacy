...
export default function MobileNavbar() {
    ...
    return (
        <div className='md:hidden fixed bottom-0 left-0 right-0 h-20 p-xs bg-background z-[99999] shadow-[0_10px_15px_0_#202020]'>
            <div className="grid gap-base items-center grid-cols-4">
                {navigationLinks.map(item => (
                    <Link className={cn("relative block p-xs", path === item.href && "text-primary bg-secondary rounded-md")} key={item.title + "MobileNav"} href={item.href}>
